...
defmodule Ecto.Integration.TypeTest do
  ...
  test "primitive types" do
    ...
    assert [1] =
             TestRepo.all(
               from(p in Post, where: p.counter == ^integer, select: p.counter)
             )

    # Integers
    assert [1] =
             TestRepo.all(
               from(p in Post, where: p.visits == ^integer, select: p.visits)
             )

    assert [1] =
             TestRepo.all(
               from(p in Post, where: p.visits == 1, select: p.visits)
             )
    ...
    assert [0.5] =
             TestRepo.all(
               from(p in Post, where: p.intensity == 0.5, select: p.intensity)
             )
    ...
    assert [true] =
             TestRepo.all(
               from(p in Post, where: p.public == ^true, select: p.public)
             )
    ...
    # assert [^text] =
    #          TestRepo.all(
    #            from(p in Post, where: p.text == <<0, 1>>, select: p.text)
    #          )

    # assert [^text] =
    #          TestRepo.all(
    #            from(p in Post, where: p.text == ^text, select: p.text)
    #          )

    # UUID
    assert [^uuid] =
             TestRepo.all(
               from(p in Post, where: p.uuid == ^uuid, select: p.uuid)
             )
    ...
    # # Datetime
    # datetime =
    #   (System.system_time(:seconds) * 1_000_000)
  ...
  test "tagged types" do
    ...
    assert [1.0] =
             TestRepo.all(from(p in Post, select: type(^"1", p.intensity)))

    # Custom wrappers
    assert [1] =
             TestRepo.all(
               from(Post, select: type(^"1", Elixir.Custom.Permalink))
             )
  ...
  #   assert [^bid] =
  #            TestRepo.all(from(c in Custom, select: type(^bid, :binary_id)))
  # end
  ...
  test "array type" do
    ...
    assert TestRepo.all(
             from(t in Tag, where: t.ints == ^[1, 2, 3], select: t.ints)
           ) == [ints]

    # Both sides interpolation
    assert TestRepo.all(
             from(t in Tag, where: ^"b" in ^["a", "b", "c"], select: t.ints)
           ) == [ints]
    ...
    assert TestRepo.all(from(t in Tag, where: 1 in t.ints, select: t.ints)) == [
             ints
           ]
  ...
  #   assert TestRepo.all(
  #            from(p in Post, where: p.id == ^post1.id, select: p.meta)
  #          ) == [%{"foo" => "bar", "baz" => "bat"}]

  #   assert TestRepo.all(
  #            from(p in Post, where: p.id == ^post2.id, select: p.meta)
  #          ) == [%{"foo" => "bar", "baz" => "bat"}]
  # end
  ...
  #   post1 =
  #     TestRepo.insert!(%Post{
  #       links: %{"foo" => "http://foo.com", "bar" => "http://bar.com"}
  #     })
  ...
  #   assert TestRepo.all(
  #            from(p in Post, where: p.id == ^post1.id, select: p.links)
  #          ) == [%{"foo" => "http://foo.com", "bar" => "http://bar.com"}]

  #   assert TestRepo.all(
  #            from(p in Post, where: p.id == ^post2.id, select: p.links)
  #          ) == [%{"foo" => "http://foo.com", "bar" => "http://bar.com"}]
  # end
  ...
  #   post =
  #     TestRepo.update!(Ecto.Changeset.change(post, meta: %{hello: "world"}))
  ...
  test "decimal type" do
    ...
    assert [^decimal] =
             TestRepo.all(
               from(p in Post, where: p.cost == ^decimal, select: p.cost)
             )

    assert [^decimal] =
             TestRepo.all(
               from(p in Post, where: p.cost == ^1.0, select: p.cost)
             )

    assert [^decimal] =
             TestRepo.all(from(p in Post, where: p.cost == ^1, select: p.cost))

    assert [^decimal] =
             TestRepo.all(from(p in Post, where: p.cost == 1.0, select: p.cost))
  ...
  test "typed aggregations" do
    ...
    assert [1] =
             TestRepo.all(from(p in Post, select: type(sum(p.cost), :integer)))

    assert [1.0] =
             TestRepo.all(from(p in Post, select: type(sum(p.cost), :float)))

    # assert [^decimal] =
    #          TestRepo.all(from(p in Post, select: type(sum(p.cost), :decimal)))
  end
  ...
  test "schemaless types" do
    ...
    assert [123] =
             TestRepo.all(from(p in "posts", select: type(p.visits, :integer)))
  end
  ...
  test "schemaless calendar types" do
    ...
    assert [_] =
             TestRepo.all(
               from(
                 p in "posts",
                 where: p.inserted_at in [^datetime],
                 select: p.inserted_at
               )
             )
    ...
    assert {1, _} =
             TestRepo.insert_all("users", [
               [inserted_at: datetime, updated_at: datetime]
             ])

    assert {1, _} = TestRepo.update_all("users", set: [inserted_at: datetime])
    ...
    assert [_] =
             TestRepo.all(
               from(
                 u in "users",
                 where: u.inserted_at in [^datetime],
                 select: u.updated_at
               )
             )
